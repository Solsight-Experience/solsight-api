...
export class TokensService {
    ...
    async filter(
        ...
        if (needsAggregation && (hasVolumeFilter || hasTxnsFilter)) {
            return this.filterWithAggregation(filter, limit, sort_by, sort_order, offset, time_frame, priceChangeColumn, SortByMap);
        }
    ...
    private async filterWithAggregation(
        ...
        return {
            tokens: tokens.map((t) => mapTokenEntityToOverviewDto(t, this.network)),
            total,
            filter_applied: filter
        };
    }
