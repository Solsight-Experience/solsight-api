...
export class PromptBuilderService {
    ...
    buildSummaryPrompt(token: Token, categoryTokens?: Token[], options: SummaryOptions = {}): string {
        ...
        if (includePriceAnalysis) {
            ...
            sections.push(`**Market Cap:** $${this.formatLargeNumber(token.marketCap)}, **FDV:** $${this.formatLargeNumber(token.fdv)}`);
            sections.push(`**Liquidity:** $${this.formatLargeNumber(token.liquidity)} (${this.formatPercentage(token.liquidityChange24h)} 24h)`);
        }

        // Trading metrics
        if (includeTradingMetrics) {
            sections.push(`\n**Volume 24h:** $${this.formatLargeNumber(token.volume24h)} (${this.formatPercentage(token.volumeChange24h)})`);
            sections.push(`**Transactions 24h:** ${this.formatNumber(token.txns24hTotal)} (Buys: ${token.txns24hBuys}, Sells: ${token.txns24hSells})`);
    ...
    private analyzePriceTrend(sparkline: number[]): string {
        ...
        const trend = change > 5 ? "Strong uptrend" : change > 1 ? "Uptrend" : change > -1 ? "Sideways" : change > -5 ? "Downtrend" : "Strong downtrend";
