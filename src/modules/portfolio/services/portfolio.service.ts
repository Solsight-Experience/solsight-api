...
export class PortfolioService {
    ...
    async getOverview(userId: string, walletAddresses?: string[], timeFrame?: string) {
        ...
        const allTokenAccounts = (
            await Promise.all(wallets.map((w) => this.solanaService.getParsedTokenAccountsByOwner(new PublicKey(w.address), true)))
        ).flat();
    ...
    async getActivities(userId: string, walletAddress?: string, type: string = "all", limit: number = 20, before?: string, from?: number, to?: number) {
        ...
        const activities = sliced.map(({ tx, addr }) => this.mapToActivity(tx, addr, solPrice, tokenMetaMap));
