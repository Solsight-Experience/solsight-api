...
export class PortfolioService {
  ...
  async getOverview(userId: string, walletAddresses?: string[], timeFrame?: string) {
    ...
    const allTokenAccounts = (
      await Promise.all(
        wallets.map((w) => this.solanaService.getParsedTokenAccountsByOwner(new PublicKey(w.address), true)),
      )
    ).flat();
  ...
  private mapToActivity(tx: any, walletAddress: string, solPrice: number, tokenMetaMap: Map<string, TokenInfo>) {
    ...
    if (isDexSwap) {
      ...
      const isDirectlyInvolved = allXfers.some(
      ...
      const effectiveTokenIn  = implicitTokenIn  ?? (accountDataTokenIn  ? { toUserAccount: walletAddress,   mint: accountDataTokenIn.mint,  tokenAmount: accountDataTokenIn.tokenAmount  } : undefined);
      const effectiveTokenOut = implicitTokenOut ?? (accountDataTokenOut ? { fromUserAccount: walletAddress, mint: accountDataTokenOut.mint, tokenAmount: accountDataTokenOut.tokenAmount } : undefined);
      ...
      if (isImplicitSwap) {
        type = 'SWAP';
        app = { name: tx.source && tx.source !== 'SYSTEM_PROGRAM' && tx.source !== 'UNKNOWN'
          ? this.formatSourceName(tx.source) : 'Jupiter', type: 'DEX', icon: '' };

        if (effectiveTokenOut) {
          token_in = { address: effectiveTokenOut.mint, symbol: getSymbol(effectiveTokenOut.mint),
            logo_uri: getLogo(effectiveTokenOut.mint), amount: effectiveTokenOut.tokenAmount, value_usd: 0 };
        ...
        if (effectiveTokenIn) {
          token_out = { address: effectiveTokenIn.mint, symbol: getSymbol(effectiveTokenIn.mint),
            logo_uri: getLogo(effectiveTokenIn.mint), amount: effectiveTokenIn.tokenAmount, value_usd: 0 };
   ...
   */
  private async enrichActivitiesWithSwapDetails(
    activities: any[],
    heliusApiKey: string,
    tokenMetaMap: Map<string, TokenInfo>,
  ): Promise<void> {
    ...
    const getSymbol = (mint: string) =>
      mint === SOL_MINT_ADDR ? 'SOL' : (tokenMetaMap.get(mint)?.symbol ?? mint.slice(0, 8));
    const getLogo = (mint: string) =>
      mint === SOL_MINT_ADDR
    ...
    const candidates = activities
      .map((a, i) => ({ a, i }))
      .filter(({ a }) => a.type === 'TRANSFER_OUT' && a.token?.address === SOL_MINT_ADDR && !!a.from);
    ...
    await Promise.all(
      candidates.map(async ({ a: activity, i: idx }) => {
        try {
          ...
          const walletData = (txDetail.accountData ?? []).find(
            (d: any) => d.account === activity.from,
          );
          if (!walletData) return;

          // Any positive tokenBalanceChange means the wallet received a token in this tx
          const received = (walletData.tokenBalanceChanges ?? []).find(
            (c: any) => parseFloat(c.rawTokenAmount?.tokenAmount ?? '0') > 0,
          );
          if (!received) return;
  ...
  async getActivitiesByAddress(
    ...
    let txs = await this.fetchWalletActivities(walletAddress, type, limit, heliusApiKey, before);
  ...
  async getPnlChartByAddress(walletAddress: string, timeFrame: string = '7d', interval: string = '1d') {
    ...
    switch (timeFrame) {
      ...
      case 'all':  startTime = now - 2 * 365 * 24 * 60 * 60 * 1000; break;
    }
    switch (interval) {
