...
export class PortfolioService {
    ...
    private async heliusPost(url: string, body: any): Promise<any> {
        const windowMs = 1000;
        const now = Date.now();
        this.heliusRequestTimestamps = this.heliusRequestTimestamps.filter((ts) => now - ts < windowMs);
        if (this.heliusRequestTimestamps.length >= this.heliusRateLimitRps) {
    ...
    private mapTokenEntityToInfo(token: Token): TokenInfo {
        return {
            ...
            extensions: token.coingeckoId ? { coingeckoId: token.coingeckoId } : undefined
        };
    }

    private async getTokenMetaByAddresses(addresses: string[]): Promise<Map<string, TokenInfo>> {
        if (addresses.length === 0) return new Map();
        const tokens = await this.tokenRepository.findBy({ address: In(addresses) });
        return new Map(tokens.map((t) => [t.address, this.mapTokenEntityToInfo(t)]));
    }

    private extractMintsFromTxs(txs: any[]): string[] {
        const mints = new Set<string>();
        for (const tx of txs) {
            const swap = tx.events?.swap;
            if (swap) {
                swap.tokenInputs?.forEach((i: any) => i.mint && mints.add(i.mint));
                swap.tokenOutputs?.forEach((o: any) => o.mint && mints.add(o.mint));
            }
    ...
    async getOverview(userId: string, walletAddresses?: string[], timeFrame?: string) {
        ...
        const [solPrice, allTokenAccounts] = await Promise.all([
            this.getSolPriceUsd(),
            Promise.all(wallets.map((w) => this.solanaService.getParsedTokenAccountsByOwner(new PublicKey(w.address), true))).then((r) => r.flat())
        ]);
        let total_balance_usd = total_balance_sol * solPrice;

        const mints = [...new Set(allTokenAccounts.map((acc) => acc.account.data.parsed.info.mint))];
        const tokenMetaMap = await this.getTokenMetaByAddresses(mints);
        ...
        const [transactionStats, trades] = await Promise.all([this.getTransactionStats(wallets, heliusApiKey), this.fetchAllTrades(wallets, heliusApiKey)]);
    ...
    private async fetchWalletActivities(walletAddress: string, type: string, limit: number, heliusApiKey: string, before?: string) {
        let heliusType = "";
    ...
    private mapToActivity(tx: any, walletAddress: string, solPrice: number, tokenMetaMap: Map<string, TokenInfo>) {
        const SOL_MINT = "So11111111111111111111111111111111111111112";
        ...
        const getLogo = (mint: string) => {
            ...
            return tokenMetaMap.get(mint)?.logoURI ?? "";
        };
            ...
            if (isImplicitSwap) {
                ...
                if (effectiveTokenOut) {
                    token_in = {
                        ...
                        symbol: getSymbol(effectiveTokenOut.mint),
                        logo_uri: getLogo(effectiveTokenOut.mint),
                        amount: effectiveTokenOut.tokenAmount,
                        value_usd: 0
                    };
                } else if (implicitSolOut) {
                    const amt = implicitSolOut.amount / LAMPORTS_PER_SOL;
                    token_in = { address: SOL_MINT, symbol: "SOL", logo_uri: getLogo(SOL_MINT), amount: amt, value_usd: amt * solPrice };
                }

                if (effectiveTokenIn) {
                    token_out = {
                        address: effectiveTokenIn.mint,
                        symbol: getSymbol(effectiveTokenIn.mint),
                        logo_uri: getLogo(effectiveTokenIn.mint),
                        amount: effectiveTokenIn.tokenAmount,
                        value_usd: 0
                    };
                } else if (implicitSolIn) {
                    const amt = implicitSolIn.amount / LAMPORTS_PER_SOL;
                    token_out = { address: SOL_MINT, symbol: "SOL", logo_uri: getLogo(SOL_MINT), amount: amt, value_usd: amt * solPrice };
                }
            } else {
                ...
                if (xfer) {
                    ...
                    if (tokenTransfer) {
                        token = {
                            ...
                            logo_uri: getLogo(tokenTransfer.mint),
    ...
    async getActivities(userId: string, walletAddress?: string, type: string = "all", limit: number = 20, before?: string, from?: number, to?: number) {
        ...
        const results = await Promise.all(
            targetAddresses.map(async (addr) => {
                const txs = await this.fetchWalletActivities(addr, type, limit, heliusApiKey, walletAddress ? before : undefined);
        ...
        const tokenMetaMap = await this.getTokenMetaByAddresses(this.extractMintsFromTxs(sliced.map(({ tx }) => tx)));

        const activities = sliced
            .map(({ tx, addr }) => this.mapToActivity(tx, addr, solPrice, tokenMetaMap))
            .filter((a): a is NonNullable<typeof a> => a !== null);

        await this.enrichActivitiesWithSwapDetails(activities, heliusApiKey, tokenMetaMap);
    ...
    async getPerformance(
        ...
        const [trades, solPrice] = await Promise.all([this.fetchAllTrades(wallets, heliusApiKey), this.getSolPriceUsd()]);
    ...
    private async fetchAllTrades(wallets: any[], apiKey: string): Promise<any[]> {
        ...
        for (const wallet of wallets) {
            ...
            while (!done && pages < MAX_PAGES_PER_WALLET) {
                pages++;
                const page = await this.fetchWalletActivities(wallet.address, "all", 100, apiKey, beforeSig);
    ...
    async getOverviewByAddress(walletAddress: string, timeFrame?: string) {
        ...
        const [solPrice, total_balance_sol, tokenAccounts] = await Promise.all([
            ...
            this.solanaService.getParsedTokenAccountsByOwner(pubkey, true)
        ]);
        const mints = [...new Set(tokenAccounts.map((acc) => acc.account.data.parsed.info.mint))];
        const tokenMetaMap = await this.getTokenMetaByAddresses(mints);
        let total_balance_usd = total_balance_sol * solPrice;
     ...
     */
    private async enrichActivitiesWithSwapDetails(activities: any[], heliusApiKey: string, tokenMetaMap: Map<string, TokenInfo>): Promise<void> {
        ...
        await Promise.all(
            candidates.map(async ({ a: activity, i: idx }) => {
                try {
                    const url = `${baseUrl}/v0/transactions?api-key=${heliusApiKey}`;
                    const { data } = await this.heliusPost(url, { transactions: [activity.tx_hash] });
    ...
    async getActivitiesByAddress(walletAddress: string, type: string = "all", limit: number = 20, before?: string, from?: number, to?: number) {
        const heliusApiKey = this.solanaService.getHeliusApiKey();
        if (!heliusApiKey) {
            return { activities: [], total: 0, summary: { total_volume_usd: 0, total_fees_usd: 0 } };
        }

        const [solPrice, rawTxs] = await Promise.all([this.getSolPriceUsd(), this.fetchWalletActivities(walletAddress, type, limit, heliusApiKey, before)]);
        let txs = rawTxs;
        if (from) txs = txs.filter((tx: any) => (tx.timestamp ?? 0) >= Number(from));
        if (to) txs = txs.filter((tx: any) => (tx.timestamp ?? 0) <= Number(to));
        const slicedTxs = txs.slice(0, limit);
        const tokenMetaMap = await this.getTokenMetaByAddresses(this.extractMintsFromTxs(slicedTxs));

        const activities = slicedTxs
    ...
    async getPnlChartByAddress(walletAddress: string, timeFrame: string = "7d", interval: string = "1d") {
        const heliusApiKey = this.solanaService.getHeliusApiKey();
        if (!heliusApiKey) return { chart_data: [] };
        ...
        if (count === 0) {
            await this.fetchWalletActivities(walletAddress, "all", 100, heliusApiKey);
        }
