...
export class UsersWalletsController {
    ...
    @Get()
    async list(@RequestCluster() cluster: Cluster, @CurrentUser() user: CurrentUserPayload): Promise<WalletsResponse> {
        return await this.walletsService.listForUser(cluster, user.id);
    }
    ...
    async getDetail(
        ...
        return await this.walletsService.getWalletByAddress(cluster, user.id, walletAddress);
    }

    @UseGuards(JwtAuthGuard)
    @Post()
    async create(@RequestCluster() cluster: Cluster, @CurrentUser() user: CurrentUserPayload, @Body() body: Partial<CreateWalletDto>) {
        const wallet = await this.walletsService.create(cluster, user.id, body as CreateWalletDto);
